...
def training(model, train_loader, n_epochs=3, validation=False, val_loader=None, print_every_epoch=True, optimizer=None):
    ...
    for idx in config.log_images_indices:
        ...
        wandb.log({"original_image_" + str(idx)
                  : [wandb.Image(original_image, caption="|Original index: " + str(idx) + "|")]})
        wandb.log({"gt_image_" + str(idx)
                  : [wandb.Image(gt_image, caption="|GT index: " + str(idx) + "|")]})
