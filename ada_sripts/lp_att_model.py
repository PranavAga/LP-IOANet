...
img_shadow_raw_train = load_images_from_folder(folder_path_shadow_train,img_train_indices)
img_removed_raw_train = load_images_from_folder(folder_path_removed_train,img_train_indices)
img_shadow_raw_test = load_images_from_folder(folder_path_shadow_test,img_test_indices)
img_removed_raw_test = load_images_from_folder(folder_path_removed_test,img_test_indices)
...
class LPNet(nn.Module):
    def __init__(self, i2it_model_path, L=2, interp_mode='nearest-exact',lr=0.1):
        ...
        self.i2it_model = nn.DataParallel(UnetWithAT(), device_ids=[0,1,2,3])
    ...
    for epoch in (range(n_epochs)):
        ...
        if validation:
            ...
            for x_val, y_val in val_loader:
                ...
                metrices['val/accuracy'] += accuracy(model.module,x_val,y_val)
...
val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=10, shuffle=False, drop_last=True)
del val_dataset
...
# print('model size: {:.3f}MB'.format(size_all_mb),flush=True)
